...
let validCount = 0;
const validWords = [];
const TARGET_WORDS = 1300; // From 1200 to 2500

for (let i = 1; i <= 10; i++) {
  const batchPath = join(import.meta.dirname, "..", "scratch", `b1-batch${i}.txt`);
...
fs.writeFileSync(join(import.meta.dirname, "data", "core-lexicon-7.txt"), out);
console.log(`Wrote ${validWords.length} new B1 words (up to rank ${currentRank - 1})`);
