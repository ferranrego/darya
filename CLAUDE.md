...
**Darya** - Dari (`prs`), deployed at daryalearn.vercel.app.

This was one repo building two apps until it was split. The sibling is
**Riera** (Catalan), at `github.com/ferranrego/riera`, and the two share
ancestry and identical file paths on purpose - see "Porting from Riera" below.
...
3. **`content/active` always points at `content/prs`.** It is written by
   ...
   cannot happen here any more. This is what the split bought: Darya and
   Riera can now run locally at the same time.
...
```bash
git fetch riera
git log --oneline main..riera/main    # what is portable
./scripts/port.sh <sha>              # cherry-pick, dropping Riera-only files
pnpm gate
```
...
feat(prs): interference rules for correction hints    <- never ports
```
...
What ports: `src/` outside `src/lib/lang/prs/`, `supabase/migrations/`, shared
`scripts/`, `content/schema/`. What never does: `content/prs/` and the profile's
...
riera/main` still works - shared ancestry makes it a real three-way merge, and
