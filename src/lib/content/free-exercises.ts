...
export function buildFreeExercises(input: FreeExerciseInput): FreeExercise[] {
  ...
  // Walk the targets in the order given - callers put unresolved mistakes
  ...
  for (const lexemeId of targetLexemeIds) {
    ...
    for (const doc of docs) {
      ...
      for (const sentence of doc.sentences) {
        ...
        if (!sentence.tokens.every((t) => !t.lexemeId || t.lexemeId === lexemeId || wanted.has(t.lexemeId) || byId.has(t.lexemeId))) {
          continue;
        }
  ...
  // Fill any remainder with word-order practice over the same sentences, which
